...
#  Computer Project #8
#
#  Dispalys an original english text
#   translate it into 7 different languages using list    
#   then again translate it using dictionary this time 
#   displays the time taken by the program to translate in both list
#   method and dictionary method
###########################################################
...
def getDictFormattedDatabases(listDatabases):
    ...
    should take the following form:
        ...
        {'have':'avoir', 'baby':'bébé', ...}
    """
    dictDatabases=[]#initialize the dictionary
    
    for languageTranslation in listDatabases:
        #languageTranslation is a tupple with language name and its enlish
        #translation dictionary
        
        tupleLanguageTransation=(languageTranslation[0],dict(languageTranslation[1]))
        #creates a tupple of language name string followed by a dicitonary of translation
        dictDatabases.append(tupleLanguageTransation)
        #appends the tupple to the dictDatabases
...
def translateFromDict(text, dictBasedDb):
    ...
    """ 
    translation = ""
    #the translation string initialization
    words = text.split()
    #list of words in the given text string
    
    for word in words:
        #gets each word from the list of words
        toAppend = ''
        #initialiizes toAppend string which is used if the word contains '.'
        if word[-1] == '.':
            #last character is a '.'
            word = word[:-1]
            ...
            #assigns the toAppen '.'
        try:
            translatedWord = dictBasedDb[word] #d
        except KeyError:
        #if the word in the word list is not having any translation in dictionary
            translatedWord = word #the translated word will be same in the 
...
def performDictBasedTranslations(preprocessedText, dictBasedDatabases):
    ...
    for tup in dictBasedDatabases:
        #tup contain the language sring and its translation to english in list
        translatedText = translateFromDict(preprocessedText, tup[1])#calls the fucntion
        display(tup[0], translatedText)#display the translated text of the language
